...
function Page() {
  ...
  const updateProfile = useMutation({
    ...
    onSuccess: () => { toast.success("Perfil atualizado"); qc.invalidateQueries({ queryKey: ["profiles"] }); },
  ...
  const updateRole = useMutation({
    ...
    onSuccess: () => { toast.success("Permissão atualizada"); qc.invalidateQueries({ queryKey: ["profiles"] }); },
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <section className="rounded-lg border border-border bg-card p-5">
          ...
          <div className="space-y-4 max-w-md">
            ...
            <div>
              ...
              <div className="mt-1.5"><Badge>{profile?.role}</Badge></div>
            </div>
            <Button onClick={() => updateProfile.mutate()} disabled={updateProfile.isPending}>
              {updateProfile.isPending ? <Loader2 className="size-4 mr-2 animate-spin" /> : <Save className="size-4 mr-2" />}
              Salvar
            </Button>
          </div>
        </section>
        ...
        {hasRole("admin") && (
          <section className="rounded-lg border border-border bg-card p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">Atribua permissões aos usuários cadastrados.</p>
            <div className="space-y-2">
              {users.data?.map((u) => (
                <div key={u.id} className="flex items-center gap-3 p-3 rounded-md border border-border">
